package com.ifudata.ic.smc.calculate.topology.core.proxy;
...
import com.ifudata.dvp.storm.sequence.datasource.SeqDataSourceLoader;
import com.ifudata.dvp.storm.sequence.util.SeqUtil;
import com.ifudata.dvp.storm.util.BaseConstants;
import com.ifudata.dvp.storm.util.HBaseProxy;
import com.ifudata.ic.smc.api.policymanage.param.StepCalValue;
import com.ifudata.ic.smc.calculate.topology.constants.SmcExceptCodeConstant;
import com.ifudata.ic.smc.calculate.topology.core.bo.StlBillData;
import com.ifudata.ic.smc.calculate.topology.core.bo.StlBillItemData;
import com.ifudata.ic.smc.calculate.topology.core.bo.StlElement;
import com.ifudata.ic.smc.calculate.topology.core.bo.StlPolicy;
import com.ifudata.ic.smc.calculate.topology.core.bo.StlPolicyItem;
import com.ifudata.ic.smc.calculate.topology.core.bo.StlPolicyItemCondition;
import com.ifudata.ic.smc.calculate.topology.core.bo.StlPolicyItemPlan;
import com.ifudata.ic.smc.calculate.topology.core.bo.StlSysParam;
import com.ifudata.ic.smc.calculate.topology.core.bo.SwitchCalValue;
import com.ifudata.ic.smc.calculate.topology.core.util.DateUtil;
import com.ifudata.ic.smc.calculate.topology.core.util.IKin;
import com.ifudata.ic.smc.calculate.topology.core.util.SftpUtil;
import com.ifudata.ic.smc.calculate.topology.core.util.SmcCacheConstant;
import com.ifudata.ic.smc.calculate.topology.core.util.SmcCacheConstant.ParamCode;
import com.ifudata.ic.smc.calculate.topology.core.util.SmcCacheConstant.TypeCode;
//import com.ifudata.ic.smc.calculate.topology.core.util.SmcCacheFactory;
import com.ifudata.ic.smc.calculate.topology.core.util.SmcConstants;
import com.ifudata.ic.smc.calculate.topology.core.util.SmcSeqUtil;
//import com.ifudata.ic.storm.jdbc.JdbcProxy;
//import com.ifudata.ic.storm.jdbc.JdbcTemplate;
//import com.ifudata.ic.storm.sequence.datasource.SeqDataSourceLoader;
//import com.ifudata.ic.storm.sequence.util.SeqUtil;
//import com.ifudata.ic.storm.util.BaseConstants;
//import com.ifudata.ic.storm.util.HBaseProxy;
...
//                .getCacheClient(SmcCacheConstant.NameSpace.POLICY_CACHE);
        String policyAll = RedisConfUtil.hget(SmcCacheConstant.NameSpace.POLICY_CACHE,
...
//        ICacheClient cacheClient = MCSClientFactory
//                .getCacheClient(SmcCacheConstant.NameSpace.POLICY_CACHE);
        String policyItemAll = RedisConfUtil.hget(SmcCacheConstant.NameSpace.POLICY_CACHE,
...
//                .getCacheClient(SmcCacheConstant.NameSpace.ELEMENT_CACHE);
        for (StlPolicyItemCondition stlPolicyItemCondition : policyItemList) {
            ...
            String elementJson = RedisConfUtil.hget(SmcCacheConstant.NameSpace.ELEMENT_CACHE,
                    elementStr.toString());
...
//                .getCacheClient(SmcCacheConstant.NameSpace.BILL_CACHE);
        ...
        String billAll = RedisConfUtil.hget(billKey, policyId);
        ...
        if (StringUtils.isBlank(billAll)) {
            ...
            RedisConfUtil.hset(billKey, policyId, JSON.toJSONString(stlBillData));
        } else {
            stlBillData = JSON.parseObject(billAll, StlBillData.class);
        }
        RedisConfUtil.hincrByFloat(assembleCacheKey(SmcCacheConstant.Cache.BILL_DATA_PREFIX, bsn),
                policyId, value);
        ...
        String billItemData = RedisConfUtil.hget(billItemKey, policyIdAndFeeItemKey);
        ...
        if (StringUtils.isBlank(billItemData)) {
            ...
            RedisConfUtil.hset(billItemKey, policyIdAndFeeItemKey, JSON.toJSONString(stlBillItemData));
        }
        RedisConfUtil.hincrByFloat(
...
//        ICacheClient billClient = MCSClientFactory
//                .getCacheClient(SmcCacheConstant.NameSpace.BILL_CACHE);
        String billAll = RedisConfUtil.get("bill");
...
//        ICacheClient billClient = MCSClientFactory
//                .getCacheClient(SmcCacheConstant.NameSpace.BILL_CACHE);
        Map<String, String> billMaps = RedisConfUtil.hgetAll(assembleCacheKey(
        ...
        if (billMaps.size() == opt_times) {
            ...
            RedisConfUtil.del(SmcCacheConstant.Cache.BILL_PREFIX + bsn);
            RedisConfUtil.del(SmcCacheConstant.Cache.BILL_ITEM_PREFIX + bsn);
            RedisConfUtil.del(SmcCacheConstant.Cache.BILL_DATA_PREFIX + bsn);
            RedisConfUtil.del(SmcCacheConstant.Cache.BILL_ITEM_DATA_PREFIX + bsn);
            RedisConfUtil.hdel(SmcCacheConstant.Cache.lockKey, bsn);
            RedisConfUtil.hdel(SmcCacheConstant.Cache.COUNTER, bsn);
        }
    }
    ...
    public boolean insert(StlBillData stlBillData, String period, String policyId, String bsn) throws Exception {
        ...
        try {
            ...
            String origFee = RedisConfUtil.hget(
    ...
    private List<StlBillItemData> getItemDataByPolicyId(String policyId, String bsn) {
        ...
        Map<String, String> itemDataMap = RedisConfUtil.hgetAll(assembleCacheKey(
...
//        ICacheClient billClient = MCSClientFactory
//                .getCacheClient(SmcCacheConstant.NameSpace.BILL_CACHE);
        Map<String, String> billAll = RedisConfUtil.hgetAll(SmcCacheConstant.Cache.BILL_PREFIX + bsn);
        StlBillData stlBillData = null;
...
//        ICacheClient client = SmcCacheFactory.getSysParamCacheClient();
        ...
        String data = RedisConfUtil.hget(SmcCacheConstant.NameSpace.SYS_PARAM_CACHE, sysParamKey);
