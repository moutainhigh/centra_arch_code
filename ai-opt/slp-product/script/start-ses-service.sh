...
LOG_PATH=${APP_HOME}/logs/ses-service.log
CLASSPATH="${CP}"
...
START_CMD="${MEM_ARGS}  ${JAVA_OPTIONS} com.ai.opt.sdk.appserver.SESServiceStart  >> $LOG_PATH & 2 > 1 &"
...
echo "-------------------${APP_NAME} ses refresh start --------------------"
java ${START_CMD}
echo "${APP_NAME} ses refresh end!! logs at $LOG_PATH"
