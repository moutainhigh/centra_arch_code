package com.ai.slp.sdk.components.sequence.dao.impl;
...
public class SequenceDaoImpl implements ISequenceDao {
    ...
    private static final String UPDATE_WHERE_SEQNAME_SQL = "update sys_sequences set LAST_NUMBER=? where SEQUENCE_NAME = ?";

    private static final String QUERY_ALL_SQL = "select SEQUENCE_NAME,LAST_NUMBER from sys_sequences";

    private static final String QUERY_WHERE_SEQNAME_SQL = "select SEQUENCE_NAME,LAST_NUMBER,JVM_STEP_BY,INCREMENT_BY from sys_sequences where SEQUENCE_NAME = ?";
    ...
    public List<Sequence> queryAllSequence() {
        ...
        try {
            ...
            ps = conn.prepareStatement(QUERY_ALL_SQL);
            rs = ps.executeQuery();
            if (rs != null) {
    ...
    public SequenceCache getSequenceCache(String sequenceName) {
        ...
        try {
            conn = db.getConnection();
            conn.setAutoCommit(false);
            ps = conn.prepareStatement(QUERY_WHERE_SEQNAME_SQL);
            ...
            if (rs != null && rs.next()) {
                ...
                rs.close();
                ps.close();
                ps = conn.prepareStatement(UPDATE_WHERE_SEQNAME_SQL);
                ...
                if (ret == 1) {
                    ...
                    LOG.debug("get sequence cache:" + JSONObject.toJSONString(cache));
                    return cache;
                }
            }
        } catch (SQLException e) {
