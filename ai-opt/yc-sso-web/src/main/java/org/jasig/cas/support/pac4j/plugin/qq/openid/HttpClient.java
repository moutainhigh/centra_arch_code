...
public class HttpClient implements Serializable {
		...
		if ((this.proxyHost != null) && (!this.proxyHost.equals(""))) {
			...
			if ((this.proxyAuthUser != null) && (!this.proxyAuthUser.equals(""))) {
				this.client.getState().setProxyCredentials(AuthScope.ANY,
						new UsernamePasswordCredentials(this.proxyAuthUser, this.proxyAuthPassword));
	...
	private Response multPartURL(String fileParamName, String url, PostParameter[] params, File file,
			boolean authenticated) throws QQConnectException {
		PostMethod postMethod = new PostMethod(url);
		try {
			Part[] parts = null;
			if (params == null)
	...
	public Response httpRequest(HttpMethod method, Boolean WithTokenHeader) throws QQConnectException {
		...
		try {
			...
			if (responseCode != 200) {
				try {
					throw new QQConnectException(getCause(responseCode), response.asJSONObject(),
	...
	private static class ByteArrayPart extends PartBase {
		...
		public ByteArrayPart(byte[] data, String name, String type) throws IOException {
			...
			super(name, type, new String(data, 0, data.length), "UTF-8");
			this.mName = name;
			this.mData = data;
		}
