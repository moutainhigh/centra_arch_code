...
public class CountyTag extends BodyTagSupport {
    /**
     * 
     */
    private static final long serialVersionUID = 1116086858080315421L;
    
    private Logger logger = Logger.getLogger(CountyTag.class);
    /* 字典相关 */
    ...
    public int doEndTag() throws JspException {
        try {
            ...
            /* 处理数据字典 */
            /*List<PostAreaVo> lstDictItem = initDatasrc();
            ...
            if (lstDictItem != null) {*/
               /* // 排序
                ...
                /*for (int i = 0; i < lstDictItem.size(); i++) {
                	PostAreaVo postAreaVo = lstDictItem.get(i);
                    strCode = "";
                    // 显示code
                    if (displayCode.equalsIgnoreCase("TRUE")) {
                        strCode = postAreaVo.getAreaCode() + "-";
                    }
                    if (postAreaVo.getAreaCode().equals(value)) {
                        strTemp += "\t<option value=\""
                                + postAreaVo.getAreaCode() + "\" selected>"
                                + strCode + postAreaVo.getAreaName()
                                + "</option>\n";
                    } else {
                        strTemp += "\t<option value=\""
                                + postAreaVo.getAreaCode() + "\">" + strCode
                                + postAreaVo.getAreaName() + "</option>\n";
                    }
                }
            }
     ...
     * @return
     */
    /*private List<PostAreaVo> initDatasrc() {
    	List<PostAreaVo> res = new ArrayList<PostAreaVo>();
    	if(StringUtils.isNotBlank(parentValue)){
    		List<PostAreaVo> vos = PostAreaUtil.getAreas(Constants.AreaLevel.COUNTY, parentValue);
            if (StringUtils.isNotEmpty(keys)) {
                String[] _keys = keys.split(",");
                for (int i = 0; i < _keys.length; i++) {
                    for(int j=0;j<vos.size();j++){
                    	PostAreaVo vo = vos.get(i);
