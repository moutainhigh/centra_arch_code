...
public class GnAreaTag extends BodyTagSupport {
    ...
    private Logger logger = Logger.getLogger(GnAreaTag.class);
    /* 字典相关 */
                ...
                for (int i = 0; i < lstDictItem.size(); i++) {
                    ...
                    if (areaVo.getAreaCode().equals(value)) {
                        strTemp += "\t<option value=\""
                                + areaVo.getAreaCode() + "\" selected>"
                                + strCode + areaVo.getAreaName()
                                + "</option>\n";
                    } else {
                        strTemp += "\t<option value=\""
                                + areaVo.getAreaCode() + "\">" + strCode
                                + areaVo.getAreaName() + "</option>\n";
                    }
                }*/
            //}
    ...
    /*private List<AreaVo> initDatasrc() {
    	...
    	if(StringUtils.isNotBlank(parentValue)){
    		List<AreaVo> vos = GnAreaUtil.getAreas(Constants.AreaLevel.CITY, parentValue);
