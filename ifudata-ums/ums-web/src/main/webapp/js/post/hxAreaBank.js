...
function setCity(ele,cityId,cityCode) {
	...
	$.ajax({
		url : webpath+'/common/postController/getAreas',
		data : {
			level : level,
			code : provinceCode
		},
 ...
 * countyCode  区县的默认选中值
 */
function setBank(ele,province,bankId,defaultCityCode,payType) {
	var city = ele.value;
	if(city!=""){
		$.ajax({
 ...
 * countyCode  区县的默认选中值
 */
function setBankByCityValue(cityValue,province,bankId,defaultCityCode,payType) {
	var city = cityValue;
	$.ajax({
		url : webpath+'/common/area/getBank',
		data : {
