...
import com.ifudata.ic.smc.preprocess.topology.core.constant.SmcConstants.StlBillItemData.FamilyColumnName;
import com.ifudata.ic.smc.preprocess.topology.core.constant.SmcConstants.StlElement.IsNecessary;
import com.ifudata.ic.smc.preprocess.topology.core.constant.SmcConstants.StlElement.IsPrimaryKey;
import com.ifudata.ic.smc.preprocess.topology.core.constant.SmcConstants.StlElement.type;
import com.ifudata.ic.smc.preprocess.topology.core.constant.SmcHbaseConstants;
...
import com.ifudata.ic.smc.preprocess.topology.core.vo.StlSysParam;
//import com.ifudata.ic.storm.failbill.FailBillHandler;
//import com.ifudata.ic.storm.jdbc.JdbcProxy;
//import com.ifudata.ic.storm.message.MappingRule;
//import com.ifudata.ic.storm.message.MessageParser;
...
//        }
    ...
    public void execute(Tuple input, BasicOutputCollector collector) {
        ...
        try {
            ...
            Long numberLong = RedisConfUtil.incr(inputData.substring(0, 20));
            ...
            // 根据对象id获取元素ID
            String elementStrings = RedisConfUtil.hget(NameSpace.OBJECT_ELEMENT_CACHE, tenantId + "."
                    + objectId);// key：租户id.流水对象id获得元素对象想
            if (StringUtil.isBlank(elementStrings)) {
                throw new BusinessException(SmcConstants.Special.NO_DATA_OR_CACAE_ERROR,
                        tenantId + "." + objectId + "租户id.流水对象id获得元素对象为空");
            }
            List<StlElement> list = JSONArray.parseArray(elementStrings, StlElement.class);
    ...
    private void increaseRedise(boolean b, String tenantId, String batchNo) {
        if (b) {
            ...
            long num = RedisConfUtil.incr(successBuilder.toString());
            ...
            long numFailed = RedisConfUtil.incr(failedBuilder.toString());
    ...
    private Boolean checkValueType(String element, StlElement stlElement) throws Exception {
        ...
        if (type.ENUM.equals(valueType)) {
            ...
            String result = RedisConfUtil.hget(NameSpace.SYS_PARAM_CACHE, stlElement.getTenantId()
