package com.ifudata.ic.smc.preprocess.topology.core.bolt;
...
import com.ifudata.ic.smc.preprocess.topology.core.constant.SmcConstants.DshmTableName;
import com.ifudata.ic.smc.preprocess.topology.core.constant.SmcConstants.NameSpace;
import com.ifudata.ic.smc.preprocess.topology.core.constant.SmcConstants.StlElement.StatisticsType;
import com.ifudata.ic.smc.preprocess.topology.core.constant.SmcExceptCodeConstant;
import com.ifudata.ic.smc.preprocess.topology.core.util.IKin;
import com.ifudata.ic.smc.preprocess.topology.core.util.LoadConfUtil;
import com.ifudata.ic.smc.preprocess.topology.core.vo.FinishListVo;
import com.ifudata.ic.smc.preprocess.topology.core.vo.StlElement;
import com.ifudata.ic.smc.preprocess.topology.core.vo.StlElementAttr;
...
//    private ICacheClient cacheClientStlObjStat;
    ...
    public void execute(Tuple input, BasicOutputCollector collector) {
        ...
        try {
            String numberLong = RedisConfUtil.hget(NameSpace.CHECK_COUNT_CACHE,
            ...
            // 根据对象id获取元素ID
            String tenantIdPolicyStrings = RedisConfUtil.hget(
            ...
            if (!StringUtil.isBlank(tenantIdPolicyStrings)) {
                ...
                for (String tenantIdpolicyId : list) {
                    ...
                    String elements = RedisConfUtil.hget(
                    ...
                    if (!StringUtil.isBlank(elements)) {
                        ...
                        for (StlElement stlElement : elements2) {
                            ...
                            String result = RedisConfUtil.hget(NameSpace.STL_OBJ_STAT, key);
                            if (StringUtil.isBlank(result)) {
                                String value = assemValue(tenantId, policyId.toString(),
                                        billTimeSn, objectId, stlElement.getElementId().toString(),
                                        "0", "0");
                                RedisConfUtil.hset(NameSpace.STL_OBJ_STAT, key, value);
                            ...
                            String elementResult = RedisConfUtil
                            ...
                            if (!StringUtil.isBlank(elementResult)) {
                                ...
                                for (StlElementAttr stlElementAttr : elementAttrlist) {
                                    ...
                                    if (StringUtil.isBlank(matchType)) {
                                        throw new BusinessException(
                                        		SmcConstants.Special.SYSTEM_ERROR,
                                ...
                                // 结算对象统计数据表 统计次数+1
                                String resultValue = RedisConfUtil.hget(
                                ...
                                if (flag) { // 如果满足则根据汇总方式进行累加 ,结算对象统计数据表的统计次数加1
                                    ...
                                    if (StatisticsType.RECORD_COUNT.equals(stlElement
                                        ...
                                        String elementVoString = RedisConfUtil.hget(
                                                NameSpace.ELEMENT_CACHE, stlElement.getTenantId()
                                                        + "." + elementIdString);
                                        if (StringUtil.isBlank(elementVoString)) {
                                            throw new BusinessException(
                                            		SmcConstants.Special.NO_DATA_OR_CACAE_ERROR,
...
//            }
            long num = RedisConfUtil.incr(countKey);
    ...
    private String getElementCode(String tenantId, Long subElementId) throws BusinessException {
        ...
        String result = RedisConfUtil.hget(NameSpace.ELEMENT_CACHE, sBuilder.toString());
        if (StringUtil.isBlank(result)) {
            throw new BusinessException(SmcConstants.Special.NO_DATA_OR_CACAE_ERROR,
    ...
    private void updateFinishRedis(String tenantId, String objectId, String billTimeSn,
            String batchNo, String totalRecord) {
        String finishKey = SmcConstants.FINISHKEY;
        String cacheStatsTimesValues = RedisConfUtil.hget(NameSpace.STATS_TIMES, finishKey);
        if (StringUtil.isBlank(cacheStatsTimesValues)) {
            ...
            RedisConfUtil.hset(NameSpace.STATS_TIMES, finishKey, JSON.toJSONString(list));
    ...
    private void increase(String resultRecord, float num, String key, boolean b) {
        ...
        RedisConfUtil.hset(NameSpace.STL_OBJ_STAT, key, resultNew.toString());
    }
    ...
    private Boolean checkRel(String matchType, String matchValue, String elementValue)
        ...
        if (matchType.equals("in")) {
            ...
            if (resultss == null) {
                throw new BusinessException(SmcConstants.Special.SYSTEM_ERROR, "a="
